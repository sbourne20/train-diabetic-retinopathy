#!/usr/bin/env python3
"""
Debug OVO Fix - Exclude Broken DenseNet121

The results show DenseNet121 is performing at 30% (broken).
This version excludes it and uses only MobileNet-v2 + Inception-v3.
"""
...
class DebugOVOEnsemble(nn.Module):
    """Debug OVO Ensemble - Exclude Broken DenseNet121"""

    def __init__(self, base_models=['mobilenet_v2', 'inception_v3'],  # EXCLUDED densenet121
        ...
        # REAL accuracies from diagnostic analysis (only good models)
        ...
        # MODERATE Class 1 boost (since we have fewer models)
        self.class_weights = torch.tensor([1.0, 6.0, 2.0, 3.0, 4.0])  # Reduced boost
        self.class1_pairs = ['pair_0_1', 'pair_1_2', 'pair_1_3', 'pair_1_4']

        logger.info(f"🐛 Debug OVO Ensemble (excluded DenseNet121)")
        ...
        for model_name, model_classifiers in self.classifiers.items():
            ...
            for classifier_name, classifier in model_classifiers.items():
                ...
                if (model_name, classifier_name) in self.weak_classifiers:
                    accuracy_weight = (base_accuracy ** 3) * 0.6  # Moderate penalty
                else:
                ...
                # Class weights with Class 1 boost
                class_a_weight = class_weights[class_a]
                class_b_weight = class_weights[class_b]

                # Class 1 boost (moderate since fewer models)
                if classifier_name in self.class1_pairs:
                    if class_a == 1:
                        class_a_weight *= 2.0  # Moderate boost
                    if class_b == 1:
                        class_b_weight *= 2.0  # Moderate boost
...
def load_debug_ensemble(results_dir, config):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logger.info(f"🎮 Using device: {device}")

    # Only use good models
    good_models = ['mobilenet_v2', 'inception_v3']

    ensemble = DebugOVOEnsemble(
        base_models=good_models,
        ...
        for class_a, class_b in combinations(range(config['data']['num_classes']), 2):
            ...
            if model_path.exists():
                try:
                    ...
                    failed_models.append(f"{model_name}_{pair_name}")
            else:
                logger.warning(f"Missing: {model_path.name}")

    logger.info(f"📦 Loaded {loaded_count}/20 binary classifiers (good models only)")
    if failed_models:
        logger.warning(f"Failed models: {failed_models}")

    return ensemble

def evaluate_debug_ensemble(ensemble, test_loader, config):
    logger.info("📋 Evaluating Debug Ensemble (no DenseNet121)")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    all_predictions = []
    all_targets = []
    individual_predictions = {model: [] for model in ['mobilenet_v2', 'inception_v3']}
...
def main():
    print("🐛 DEBUG OVO ENSEMBLE (EXCLUDE BROKEN DENSENET121)")
    print("=" * 60)
    ...
    if config_path.exists():
        ...
        config = {
            'model': {'base_models': ['mobilenet_v2', 'inception_v3']},  # Excluded densenet121
            'data': {'num_classes': 5, 'img_size': 224}
        }

    logger.info("🏗️ Loading debug ensemble (no DenseNet121)...")
    ensemble = load_debug_ensemble(results_dir, config)
    ...
    logger.info("🐛 Evaluating debug ensemble...")
    results = evaluate_debug_ensemble(ensemble, test_loader, config)

    print("\n" + "="*60)
    print("🏆 DEBUG RESULTS (NO DENSENET121)")
    print("="*60)
    ...
    # Progress tracking
    print(f"\n📈 COMPARISON:")
    print(f"   Original (3 models): ~80.9%")
    print(f"   Broken Fix (3 models): 76.6% (DenseNet121 broken)")
    print(f"   Debug Fix (2 models): {results['ensemble_accuracy']*100:.1f}%")

    print(f"\n🔍 DIAGNOSIS:")
    if results['ensemble_accuracy'] > 0.80:
        print("   ✅ DenseNet121 was the problem! 2-model ensemble works.")
        print("   💡 Recommendation: Fix DenseNet121 loading or exclude it")
    else:
        print("   ⚠️ Still issues with 2-model ensemble")
        print("   💡 Recommendation: Check all model loading")
