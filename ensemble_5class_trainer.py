#!/usr/bin/env python3
"""
5-Class Ensemble Trainer for Diabetic Retinopathy Classification

Full ICDR 5-class implementation optimized for perfectly balanced datasets and medical-grade
accuracy (95%+). Supports clinical findings extraction and OVO ensemble methodology.

Key Features:
- 5-class classification: 0 (No DR), 1 (Mild NPDR), 2 (Moderate NPDR), 3 (Severe NPDR), 4 (PDR)
...
- Weighted voting strategy with severity-based boost
- Enhanced preprocessing with CLAHE and medical-grade augmentation
- PERFECTLY balanced dataset (10,787 images per class - 1.00:1 ratio)
- Clinical findings extraction for medical reporting
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='5-Class Multi-Architecture Ensemble DR Training (Full ICDR Classification)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example Usage:
  # Basic 5-class ensemble training
  python ensemble_5class_trainer.py --mode train --dataset_path ./dataset_eyepacs_5class_balanced --epochs 50

  # Medical-grade 5-class training with full features
  ...
  python ensemble_5class_trainer.py --mode train --dataset_path ./dataset_eyepacs_5class_balanced \\
    ...
    # Dataset configuration (5-CLASS SPECIFIC)
    parser.add_argument('--dataset_path', default='./dataset_eyepacs_5class_balanced',
                       help='Path to 5-class dataset directory (train/val/test structure)')
    parser.add_argument('--num_classes', type=int, default=5,
                       help='Number of classes (5 for full ICDR: 0-4)')
    ...
    parser.add_argument('--class_weight_0', type=float, default=1.0,
                       help='Class weight for Class 0 (No DR) - perfectly balanced dataset')
    parser.add_argument('--class_weight_1', type=float, default=1.0,
                       help='Class weight for Class 1 (Mild NPDR) - perfectly balanced dataset')
    parser.add_argument('--class_weight_2', type=float, default=1.0,
                       help='Class weight for Class 2 (Moderate NPDR) - perfectly balanced dataset')
    parser.add_argument('--class_weight_3', type=float, default=1.0,
                       help='Class weight for Class 3 (Severe NPDR) - perfectly balanced dataset')
    parser.add_argument('--class_weight_4', type=float, default=1.0,
                       help='Class weight for Class 4 (PDR) - perfectly balanced dataset')
    ...
    # Experiment settings (5-CLASS SPECIFIC)
    parser.add_argument('--experiment_name', default='5class_densenet121_balanced',
                       help='Experiment name for logging')
    parser.add_argument('--output_dir', default='./densenet_5class_results',
    ...
    # Medical-grade validation (5-CLASS SPECIFIC)
    ...
    parser.add_argument('--target_accuracy', type=float, default=0.95,
                       help='Target accuracy for 5-class (95%% for medical-grade)')
...
class OVOEnsemble(nn.Module):
    ...
    def forward(self, x, return_individual=False):
        ...
        # Medical-grade class weights for 5-class PERFECTLY BALANCED dataset
        class_weights = torch.tensor([1.0, 1.0, 1.0, 1.0, 1.2], device=device)  # Classes 0-4, slight PDR boost
...
def setup_ovo_experiment(args):
    ...
    config = {
        ...
        'training': {
            ...
            'class_weight_3': args.class_weight_3,
            'class_weight_4': args.class_weight_4,
            'resume': args.resume
        },
        'paths': {
...
def train_multiclass_dr_model(model, train_loader, val_loader, config, model_name):
    ...
    if config['training']['enable_focal_loss']:
        ...
        if config['training']['enable_class_weights']:
            # 5-CLASS PERFECTLY BALANCED WEIGHTS - all equal for balanced dataset
            weight_0 = config['training'].get('class_weight_0', 1.0)
            weight_1 = config['training'].get('class_weight_1', 1.0)
            weight_2 = config['training'].get('class_weight_2', 1.0)
            weight_3 = config['training'].get('class_weight_3', 1.0)
            weight_4 = config['training'].get('class_weight_4', 1.0)
            class_weights = torch.tensor([weight_0, weight_1, weight_2, weight_3, weight_4])
            criterion = nn.CrossEntropyLoss(weight=class_weights.to(device), label_smoothing=label_smoothing)
            logger.info(f"✅ Using 5-CLASS weighted CrossEntropyLoss: [{weight_0}, {weight_1}, {weight_2}, {weight_3}, {weight_4}]")
            if label_smoothing > 0:
...
def main():
    ...
    print("🔢 5-CLASS ENSEMBLE DIABETIC RETINOPATHY TRAINING")
    print("=" * 70)
    print("Research Implementation: Medical-Grade 5-Class ICDR Classification")
    print("Classes: 0 (No DR), 1 (Mild NPDR), 2 (Moderate NPDR), 3 (Severe NPDR), 4 (PDR)")
    print("Base Model: DenseNet121 (optimized for perfectly balanced dataset)")
    print("Binary Classifiers: 10 (One-vs-One for 5 classes)")
    print("Target: 95%+ accuracy with medical-grade validation")
    print("Dataset: 53,935 images (PERFECTLY BALANCED - 10,787 per class)")
    print("=" * 70)
