#!/usr/bin/env python3
"""
3-Class Ensemble Trainer for Diabetic Retinopathy Classification

Specialized implementation for 3-class DR classification (NORMAL, NPDR, PDR) optimized
for balanced datasets and medical-grade accuracy. Adapted from research-validated OVO
ensemble methodology to handle simplified clinical classification.

Key Features:
- 3-class classification: NORMAL (0), NPDR (1-3 merged), PDR (4→2)
- OVO binarization approach: 3 binary classifiers for 3-class DR problem
- Transfer learning with partial fine-tuning for medical imaging
- Majority voting strategy for multi-class prediction
- Enhanced preprocessing with CLAHE and medical-grade augmentation
- Class balancing optimized for 6.45:1 imbalance ratio (NORMAL:PDR)
- Medical-grade validation with 95%+ accuracy target
- V100 16GB GPU optimized (batch size, memory management)
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='3-Class Multi-Architecture Ensemble DR Training (NORMAL, NPDR, PDR)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example Usage:
  # Basic 3-class ensemble training
  python ensemble_3class_trainer.py --mode train --dataset_path ./dataset_eyepacs_3class_balanced --epochs 50

  # Medical-grade 3-class training with full features
  python ensemble_3class_trainer.py --mode train --dataset_path ./dataset_eyepacs_3class_balanced \\
    --epochs 100 --enable_clahe --enable_focal_loss --enable_class_weights \\
    --validation_frequency 1 --checkpoint_frequency 5 --output_dir ./densenet_3class_results

  # Resume from checkpoint
  python ensemble_3class_trainer.py --mode train --dataset_path ./dataset_eyepacs_3class_balanced \\
    --resume_from_checkpoint ./densenet_3class_results/checkpoints/ensemble_best.pth
  ...
  python ensemble_3class_trainer.py --mode train --dataset_path ./dataset_eyepacs_3class_balanced \\
    ...
    # Dataset configuration (3-CLASS SPECIFIC)
    parser.add_argument('--dataset_path', default='./dataset_eyepacs_3class_balanced',
                       help='Path to 3-class dataset directory (train/val/test structure)')
    parser.add_argument('--num_classes', type=int, default=3,
                       help='Number of classes (3 for simplified DR: NORMAL, NPDR, PDR)')
    ...
    parser.add_argument('--class_weight_normal', type=float, default=0.515,
                       help='Class weight for NORMAL (Class 0) - based on 6.45:1 ratio')
    parser.add_argument('--class_weight_npdr', type=float, default=1.323,
                       help='Class weight for NPDR (Class 1) - merged 1-3')
    parser.add_argument('--class_weight_pdr', type=float, default=3.321,
                       help='Class weight for PDR (Class 2) - most critical minority class')
    ...
    # Experiment settings (3-CLASS SPECIFIC)
    parser.add_argument('--experiment_name', default='3class_densenet121_balanced',
                       help='Experiment name for logging')
    parser.add_argument('--output_dir', default='./densenet_3class_results',
    ...
    # Medical-grade validation (3-CLASS SPECIFIC)
    ...
    parser.add_argument('--target_accuracy', type=float, default=0.95,
                       help='Target accuracy for 3-class (95%% for medical-grade)')
...
class OVOEnsemble(nn.Module):
    ...
    def forward(self, x, return_individual=False):
        ...
        # Medical-grade class weights for 3-class (PDR critical minority class boost)
        class_weights = torch.tensor([0.515, 1.323, 3.321], device=device)  # NORMAL, NPDR, PDR
        pdr_pairs = ['pair_0_2', 'pair_1_2']  # Pairs involving PDR (class 2)

        # Binary accuracy weights for 3-class (will be updated after each training)
        binary_accuracies = {
            'mobilenet_v2': {
                'pair_0_1': 0.90, 'pair_0_2': 0.92, 'pair_1_2': 0.88
            },
            'inception_v3': {
                'pair_0_1': 0.90, 'pair_0_2': 0.91, 'pair_1_2': 0.87
            },
            'densenet121': {
                'pair_0_1': 0.91, 'pair_0_2': 0.93, 'pair_1_2': 0.89
            }
        }

        # No weak classifiers expected for 3-class balanced dataset
        ...
        for model_name, model_classifiers in self.classifiers.items():
            ...
            for classifier_name, classifier in model_classifiers.items():
                ...
                # FIXED: Medical-grade class weighting for 3-class
                class_a_weight = class_weights[class_a]
                class_b_weight = class_weights[class_b]

                # PDR (class 2) critical minority boost for medical safety
                if classifier_name in pdr_pairs:
                    if class_a == 2:
                        class_a_weight *= 2.0  # Boost PDR detection
                    if class_b == 2:
                        class_b_weight *= 2.0  # Boost PDR detection
...
def setup_ovo_experiment(args):
    ...
    config = {
        ...
        'training': {
            ...
            'class_weight_npdr': args.class_weight_npdr,
...
def train_multiclass_dr_model(model, train_loader, val_loader, config, model_name):
    ...
    if config['training']['enable_focal_loss']:
        ...
        if config['training']['enable_class_weights']:
            # 3-CLASS BALANCED WEIGHTS - optimized for 6.45:1 imbalance ratio
            normal_weight = config['training'].get('class_weight_normal', 0.515)
            npdr_weight = config['training'].get('class_weight_npdr', 1.323)
            pdr_weight = config['training'].get('class_weight_pdr', 3.321)
            class_weights = torch.tensor([normal_weight, npdr_weight, pdr_weight])
            criterion = nn.CrossEntropyLoss(weight=class_weights.to(device), label_smoothing=label_smoothing)
            logger.info(f"✅ Using 3-CLASS weighted CrossEntropyLoss: [{normal_weight} (NORMAL), {npdr_weight} (NPDR), {pdr_weight} (PDR)]")
            if label_smoothing > 0:
...
def main():
    ...
    print("🔢 3-CLASS ENSEMBLE DIABETIC RETINOPATHY TRAINING")
    print("=" * 70)
    print("Research Implementation: Medical-Grade 3-Class Classification")
    print("Classes: NORMAL, NPDR (merged 1-3), PDR")
    print("Base Model: DenseNet121 (optimized for balanced dataset)")
    print("Binary Classifiers: 3 (One-vs-One for 3 classes)")
    print("Target: 95%+ accuracy with medical-grade validation")
    print("Dataset: 39,850 images (balanced 6.45:1 ratio)")
    print("=" * 70)
    ...
    try:
        if args.mode == 'train':
            # ALWAYS use OVO ensemble training (even for single model)
            # This ensures consistent methodology and enables incremental model addition
            num_models = len(config['model']['base_models'])
            num_classes = config['data']['num_classes']
            num_binary_classifiers = num_models * (num_classes * (num_classes - 1) // 2)

            logger.info("🔢 Using OVO Ensemble Training")
            logger.info(f"   Base models: {config['model']['base_models']}")
            logger.info(f"   Classes: {num_classes}")
            logger.info(f"   Binary classifiers per model: {num_classes * (num_classes - 1) // 2}")
            logger.info(f"   Total binary classifiers: {num_binary_classifiers}")
